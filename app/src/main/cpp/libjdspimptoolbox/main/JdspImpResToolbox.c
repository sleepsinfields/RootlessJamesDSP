...
static const float HALF_PI = 1.57079632679489f; 
...
static void frac_circshift(
    float *dst,
    const float *src,
    int len,
    float shiftSamples
) {
    // Positive shiftSamples = delay (move content ?right? in time).
    // Circular interpolation: output[n] = src[(n - shift) mod len].

    if (len <= 0) return;

    // Wrap shift to [0, len)
    float s = fmodf(shiftSamples, (float)len);
    if (s < 0.0f) s += (float)len;

    int   intShift  = (int)floorf(s);
    float fracShift = s - (float)intShift;

    // Pure integer shift
    if (fabsf(fracShift) < 1e-6f) {
        for (int n = 0; n < len; n++) {
            int srcIndex = n - intShift;
            while (srcIndex < 0)      srcIndex += len;
            while (srcIndex >= len)   srcIndex -= len;
            dst[n] = src[srcIndex];
        }
        return;
    }

    // Fractional case: linear interpolation
    for (int n = 0; n < len; n++) {
        float pos = (float)n - s;
...
float endCutdB   = ((float)javaAdvSetPtr[1]) / ADV_DB_SCALE;
    ...
    if (convMode == 1)
    {
        // ---- Time-domain convolution path ----
        // Trim based on start/end dB
        ...
        for (i = 0; i < channels; i++)
        {
            ...
            if (temp != NULL) {
                frac_circshift(temp, chanBuf, xLen, shiftSamples);
                memcpy(chanBuf, temp, sizeof(float) * (size_t)xLen);
                free(temp);
            }
        }
    }
    ...
    {
        // ---- FFT / MPS convolution path ----
        // Initial range for MPS
        range[0] = 0;
        range[1] = alloc * 2;
        xLen = range[1] - range[0];
        ...
        if (spawnNthread < 0) spawnNthread = 0;
        int taskPerThread = (spawnNthread + 1) > 0 ? (channels / (spawnNthread + 1)) : channels;

        pthread_t *pthread = NULL;
        mpsThread *th = NULL;

        if (spawnNthread > 0) {
            pthread = (pthread_t*)malloc((size_t)spawnNthread * sizeof(pthread_t));
            th      = (mpsThread*)malloc((size_t)spawnNthread * sizeof(mpsThread));
        ...
        // Main thread handles the first chunk of channels
        int mainMax = (spawnNthread > 0) ? taskPerThread : channels;
        for (i = 0; i < mainMax && i < (int)channels; i++)
        ...
        // Now cut again based on dB, then apply fractional shift
        checkStartEnd(splittedBuffer, channels, alloc, startCutdB, endCutdB, range);
        xLen = range[1];

        for (i = 0; i < channels; i++)
        {
            // MPS output for this channel, starting at index 0
            float *chanBuf = &splittedBuffer[i][0];
            outPtr[i] = chanBuf;
