...
void VTProcess(VacuumTube *tb, float *x1, float *x2, float *out1, float *out2, size_t n)
{
    ...
    if (tb->needOversample)
    {
        for (size_t i = 0; i < n; i++)
        {
            ...
            for (int j = 0; j < tb->smp[0].factor; j++)
            {
                ...
                upsample[0][j] = (float)(bandCh1[0]
                    + (harmonic2Ch1 + harmonic3Ch1 + harmonic4Ch1 + harmonic5Ch1) * 0.2
                    + allpassCh1 + bandCh1[5]);
                upsample[1][j] = (float)(bandCh2[0]
                    + (harmonic2Ch2 + harmonic3Ch2 + harmonic4Ch2 + harmonic5Ch2) * 0.2
                    + allpassCh2 + bandCh2[5]);
            }
    ...
    else
    {
        for (size_t j = 0; j < n; j++)
        {
            ...
            out1[j] = (float)(bandCh1[0]
                + (harmonic2Ch1 + harmonic3Ch1 + harmonic4Ch1 + harmonic5Ch1) * 0.25
                + allpassCh1 + bandCh1[5]) * tb->postgain;
            out2[j] = (float)(bandCh2[0]
                + (harmonic2Ch2 + harmonic3Ch2 + harmonic4Ch2 + harmonic5Ch2) * 0.25
                + allpassCh2 + bandCh2[5]) * tb->postgain;
        }
    }
}
